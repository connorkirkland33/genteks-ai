...
| Phase 5 | 🔜 Planned | Server hardening — HTTPS, domain name, nginx reverse proxy |
| Phase 6 | 🔜 Future | VPS worker agents — multi-agent orchestration, AutoTask integration, Datto RMM integration |

### Phase 4 Detail
Phase 4 will wire up the `chat_sessions` and `chat_messages` tables that are already in the schema and database. The `web_api.py` will gain new endpoints for saving and retrieving chat history, and `dashboard.html` will switch from localStorage to API calls for all chat history operations. This enables:
